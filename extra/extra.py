...
def login():
    ...
    def login_main():
        ...
        y es llamada cuando la contraseña es correcta del usuario es igual a 'm1s10nt1c'

        Returns:
            [bool]: Estado de resultado que determina el futuro flujo del programa
        """

        def promedio_latitudes(cantidad):
            """Permite sumar una cantidad de valores que ingresa el usuario
            y muestra el resultado de la suma de dichos valores por pantalla

            Args:
                cantidad (int): Cantidad de valores a solicitar al usuario
            """

            valores = []
            for i in range(cantidad):
                valores[i] = input(f"\nIngresa el valor numero {i + 1} de la latitud: \n>> ")

            print(f"\nLa suma de las longitudes es de: {sum(valores) / len(valores)}")
        ...
        if i_user == g_username:
            ...
            # Ejecucion de la funcion de promedio de latitudes cuando la contraseña sea igual a 'm1s10nt1c'
            elif i_passwd == "m1s10nt1c":
                respuesta = int(input("\nCuantos valores de latitudes quiere ingresar a continuacion? \n>> "))
                promedio_latitudes(cantidad=respuesta)

            else:
                return False

        # Se imprime un mensaje por pantalla cuando el nombre de usuario ingresado sea igual a 'Tripulante2022'
        elif i_user == "Tripulante2022":
            error(frase="Este fue mi primer programa y vamos por más", salir=True)
...
def calc_hemisferio():
    """Permite calcular la latitud del hemisferio en el que se encuentra el usuario"""

    latitud = float(input("\nDame una latitud  y te diré cual hemisferio es... "))

    if latitud > 0:
        print("\nUsted está en hemisferio norte")

    else:
        print("\nUsted está en hemisferio sur")


def huso_horario():
    """Calcula el uso horario de una coordenada ingresada por el usuario"""

    longitud = float(input("\nEscribe una la coordenada de una longitud en Sudamérica y te diré su huso horario "))

    if longitud > -67.401 and longitud < -81.296:
        print("\nEl huso horario es -5")

    elif longitud > -52.316 and longitud < -67.402:
        print("\nEl huso horario es -4")


    elif longitud > -35.833 and longitud < -54.316:
        print("\nEl huso horario es -3")
